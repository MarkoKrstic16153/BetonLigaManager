...
export class DodajutakmicuComponent implements OnInit {
  ...
  daLiSuEkipeIste():Boolean{
    if(this.timDomaciControl.value==this.timGostiControl.value && this.timGostiControl.value!="")
      return true;
    else
      return false;
  }
  ...
  timoviSelectChange(){
    if(!(this.daLiEkipeNisuIzabrane() || this.daLiSuEkipeIste())){
      ...
      this.igracService.getIgraciTim(this.timDomaciControl.value).subscribe((podaci)=>{
        this.sviIgraciDomaci=podaci.data.Tim[0].igraci;
        console.log(this.sviIgraciDomaci)
      });
      this.igracService.getIgraciTim(this.timGostiControl.value).subscribe((podaci)=>{
