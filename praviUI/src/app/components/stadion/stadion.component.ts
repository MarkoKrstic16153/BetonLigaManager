...
export class StadionComponent implements OnInit {
  ...
  constructor(private location:Location,private stadionService:StadionService,private activeRoute:ActivatedRoute) { }

  ngOnInit() {
    this.activeRoute.params.subscribe(params=>{
      this.stadionService.getStadion(params.nazivStadiona).subscribe(({data,loading})=>{
        this.loadingStadion=loading;
        this.stadion=data.Stadion[0];
        console.log(this.stadion);
        this.stadionService.getSlicnePoKapacitetu(this.stadion.kapacitet-10000,this.stadion.kapacitet+10000).subscribe(({data,loading})=>{
          this.loadingSlicni=loading;
          console.log(data.Stadion);
          this.slicniStadioni=data.Stadion.slice(0,5);
        });
      });
    });
  }
