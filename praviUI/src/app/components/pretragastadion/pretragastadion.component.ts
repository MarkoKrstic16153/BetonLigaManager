...
export class PretragastadionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.imeControl.valueChanges
      .pipe(filter(podatak => podatak.length > 2))
      .subscribe(pocetakStadiona => {
        console.log(pocetakStadiona);
        this.stadionService
          .getStadionStartsWith(
            pocetakStadiona.charAt(0).toUpperCase() + pocetakStadiona.slice(1)
          )
          .subscribe(data => {
    ...
    this.stadionService.getStadionUtakmice().subscribe(({ data }) => {
      this.najviseUtakmica = data.Stadion.sort(function(a, b) {
        return b.utakmica.length - a.utakmica.length;
      }).slice(0, 3);
      console.log(data);
    });
  }
  ...
  pretragaStadiona($tim) {
    this.stadionService.getTimStadion($tim).subscribe(({ data, loading }) => {
      this.router.navigate([
        "/pretrazistadione/stadion/",
        data.Tim[0].stadion[0].Stadion.naziv
      ]);
    });
  }
}
