...
export class PretragaigracComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    .pipe(
      ...
      this.obsPocetakImena=this.igracService.getIgracStartsWith(pocetakImena.charAt(0).toUpperCase() + pocetakImena.slice(1));
      this.obsPocetakImena.subscribe((data)=>{
        this.rezultatPretrage=data.data.Igrac;
        this.modPretrage=" po imenu :";
      })
    });
    this.pozicijaControl.valueChanges.pipe(
    ).subscribe((pozicija)=>{
      this.obsPozicija= this.igracService.getIgracByPosition(this.pozicijaControl.value);
